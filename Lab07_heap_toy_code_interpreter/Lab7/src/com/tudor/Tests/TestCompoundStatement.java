...
import com.tudor.Model.Statements.CompoundListStatement;
...
public class TestCompoundStatement {
    ...
    @Test
    public void shouldDoCompoundListStatement__StatementsArePushedOntoStack() {
        Statement statement1 = new VariableDeclarationStatement("1",new IntType());
        Statement statement2 = new VariableDeclarationStatement("2",new IntType());
        Statement statement3 = new VariableDeclarationStatement("3",new IntType());
        Statement statement4 = new VariableDeclarationStatement("4",new IntType());

        Statement compoundListStatement =
                new CompoundListStatement(Arrays.asList(statement1,statement2,statement3,statement4));

        IStack<Statement> executionStack = new MyStack<>();
        IDict<String, Value> symbolTable = new MyDict<>();
        IList<Value> out = new MyList<>();

        executionStack.push(compoundListStatement);

        ProgramState myProgramState = new ProgramState(executionStack, symbolTable, out);

        compoundListStatement.execute(myProgramState);

        Assertions.assertEquals(statement1,executionStack.pop());
        Assertions.assertEquals(statement2,executionStack.pop());
        Assertions.assertEquals(statement3,executionStack.pop());
        Assertions.assertEquals(statement4,executionStack.pop());
    }
}
